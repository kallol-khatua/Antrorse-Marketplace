...
const ShirtProductDetails = () => {
  ...
  useEffect(() => {
    const getProductDetails = async () => {
      try {
        ...
        }/search-product/clothing/shirts/view-details?productId=${productId}`;
        ...
        if (response.status === 200) {
          ...
          for (let i = 0; i < response.data.product[0].variants.length; i++) {
            // console.log(response.data.product[0].variants[i]);
            if (response.data.product[0].variants[i][0]._id === variantId) {
              setproductVariant(response.data.product[0].variants[i]);
              setSizeVariant(response.data.product[0].variants[i][0]);
              break;
            }
          }
        }
  ...
  const getSubTitle = () => {
    let subtitle = productDetails?.ideal_for;
    if (productDetails?.fit) {
      subtitle += ` ${productDetails.fit}`;
    }
    if (productDetails?.fabric) {
      subtitle += ` ${productDetails.fabric}`;
    }
    if (productDetails?.generic_name) {
      subtitle += ` ${productDetails.generic_name}`;
    }
    return subtitle;
  };
  ...
  // console.log(sizeVariant?.sizeVariants?.inventory[0]?.available_quantity);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const data = {
        ...
        variant_id: sizeVariant._id,
        size_variant_id: sizeVariant.sizeVariants._id,
        size: sizeVariant.sizeVariants.size,
      };
  ...
  const handleSetSizeVariant = (productvariant) => {
    setSizeVariant(productvariant);

    if (
      productvariant?.sizeVariants?.inventory[0]?.available_quantity < quantity
    ) {
      setQuantity(1);
    }
  };
  ...
  const handleBuynow = () => {
    ...
    const size = sizeVariant.sizeVariants.size;
    navigate(
      `/clothings/shirts/view-details/buy-now?productId=${productId}&variantId=${variantId}&quantity=${quantity}&size=${size}&sizeVariantId=${size_variant_id}`
    );
  };
        ...
        <div className="max-w-7xl px-4 mx-auto">
          <div className="flex flex-wrap mb-24 -mx-6">
            <div className="w-full px-4 mb-8 md:w-1/2 md:mb-0 ">
              <div className="sticky flex-wrap-reverse top-0 w-full flex overflow-hidden  m-4">
                <div className="flex-wrap w-full md:w-1/4 lg:flex-col mx-2 flex">
                  {images.map((imageUrl, index) => (
                    <div key={index} className="grid grid-cols-4 p-2 sm:w-full">
                      <a className="block border border-gray-200 hover:border-sky-400">
                        <img
                          className="object-contain w-full h-16 lg:h-28"
                          src={imageUrl}
                          alt=""
                        />
                      </a>
            ...
            <div className="w-full px-4 md:w-1/2">
              <div className="lg:pl-8">
                <div className="mb-6 ">
                  <h2 className="max-w-xl mt-6 mb-3 text-xl font-semibold leading-loose tracking-wide  md:text-2xl ">
                    {productDetails?.brand_name}
                  </h2>
                  <h2 className="max-w-xl mb-3 text-xl font-semibold leading-loose tracking-wide  md:text-2xl ">
                    {getSubTitle()}
                  </h2>
                  ...
                  <p className="inline-block text-2xl font-semibold text-red-800  ">
                    <span>Rs {sizeVariant?.sizeVariants?.offered_price}</span>
                    <span className="ml-3 text-base font-normal text-gray-500 line-through ">
                      Rs {sizeVariant?.sizeVariants?.actual_price}
                    </span>{" "}
                    <span className="text-base">
                      (
                      {calculateDiscount(
                        sizeVariant?.sizeVariants?.actual_price,
                        sizeVariant?.sizeVariants?.offered_price
                      )}
                      % OFF)
                    </span>
                  </p>
                ...
                <div className="mb-4">
                  <div className="flex">
                    ...
                    <span>{productDetails?.variants?.length}</span>
                  </div>

                  {productvariant[0]?.primary_color && (
                    <div className="flex">
                      <h2 className="mb-1 mr-8 text-md font-bold ">
                        Primary Color :
                      </h2>
                      <span>{productvariant[0]?.primary_color}</span>
                    </div>
                  )}
                  {productvariant[0]?.secondary_color && (
                    <div className="flex">
                      <h2 className="mb-1 mr-8 text-md font-bold ">
                        Secondary Color :
                      </h2>
                      <span>{productvariant[0]?.secondary_color}</span>
                    </div>
                  )}
                ...
                <div className=" mb-4 flex">
                  ...
                  <div className="flex -mb-2 gap-2">
                    {productvariant.map((productvariant, index) => {
                      return (
                        <div key={index} className="flex flex-col">
                          <button
                            key={index}
                            className={`py-1 mb-2 mr-1 border w-11 hover:border-blue-400 hover:text-blue-600 ${
                              productvariant.sizeVariants._id ===
                                sizeVariant.sizeVariants._id &&
                              "border-blue-400 "
                            }`}
                            onClick={() => handleSetSizeVariant(productvariant)}
                          >
                            {productvariant.sizeVariants.size}
                          </button>
                          {productvariant.sizeVariants.inventory[0]
...
export default ShirtProductDetails;
